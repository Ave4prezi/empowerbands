...
def send_safe_notification(name, phones, emails, band_id):
  ...
  # --- BEGIN: Bulk pre-program admin pages ---

  # already installed with Flask; used to avoid HTML injection
...
def admin_preprogram():
    ...
    if bulk_db is None:
        return "<h1>Bulk band database is unavailable.</h1><p><a href=\"/dashboard\">Back to Dashboard</a></p>", 503

    bands = bulk_db.list_bands(
    status="unassigned",
    limit=1000
)
...
def api_activate():
    ...
    phone = (data.get('phone') or '').strip()
    profile_type = (data.get('profileType') or '').strip()
    activation_code = (data.get('activationCode') or '').strip()
    ...
    for row in data_rows:
        ...
        if current_band_id == band_id:
            ...
            # Prevent an existing customer profile from being overwritten
            existing_name = row[1].strip()
            existing_email = row[2].strip()
            ...
            # Connect customer information to this Safety ID
            row[1] = f'{first_name} {last_name}'.strip()
            row[2] = email
            row[3] = phone
            row[6] = profile_type
    ...
    # Do not allow someone to invent a random Safety ID
    if not band_found:
        return jsonify({
            'error': 'Safety ID not found.'
        }), 404
